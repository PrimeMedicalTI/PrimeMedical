...
//----------------------------------------------------------- Add 05/07 junto com o patch PACOTE_DE_CORRECAO_17332052_DSERGTP-9057_12.1.2210.ZIP
/*/{Protheus.doc} CTeOS_V4()
...
static function XmlCTeVPrest()
	...
	If SE1->(dbSeek(xFilial("SE1")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_SERIE+SF2->F2_DOC))
		While !SE1->(Eof()) .And. xFilial("SE1") == SE1->E1_FILIAL .And.;
			...
			If 'NF' $ SE1->E1_TIPO
				nTotRet+=SumAbatRec(SE1->E1_PREFIXO,SE1->E1_NUM,SE1->E1_PARCELA,SE1->E1_MOEDA,"V",SE1->E1_BAIXA,@nTotAbImp,@nIrRet,@nCsllRet,@nPisRet,@nCofRet,@nInssRet)
			EndIf
			SE1->(DbSkip ())
		EndDo
	EndIf
...
Endif
	...
	cRet := Alltrim(cRet) + alltrim(cImpFed) + alltrim(cImpEst) + alltrim(cImpMun) 
	nSomaImp := nValPIS + nValCOF + nValICM + nValINS 
	If nSomaImp > 0 
		cRet := "Retenção de valor da NF: R$ "+ alltrim(Transform(nValINS,"@E 999,999,999,999.99"))  
		cRet += " conforme " + RetArtigo() + ". " + chr(13) + chr(10)
...
/*/{Protheus.doc} RetArtigo
Retorna o artigo da Lei a respeito do CTeOS
@type function
@author Fernando Radu Muscalu
@since 08/03/2023
@version 1.0
@return cArtigo, caractere, "IN/RFB n° 2110/2022, Inciso I, Art. 117"
@example
(examples)
@see (links_or_references)
/*/
Static Function RetArtigo()
	
	Local cArtigo := ""

	cArtigo := Iif ( FindFunction("GTPARTCTEOS"),GTPArtCTeOS(),"IN RFB971/09 art 122" )	 	
