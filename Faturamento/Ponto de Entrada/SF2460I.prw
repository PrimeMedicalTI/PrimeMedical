...
/*/{Protheus.doc} Ponto de Entrada SF2460I - AtualizaÁ„o das tabelas referentes ‡ nota fiscal
description Ponto de entrada localizado apÛs a atualizaÁ„o das tabelas referentes ‡ nota fiscal (SF2/SD2). O Ponto de entrada È executado apÛs a exibiÁ„o da tela de contabilizaÁ„o On-Line, mas antes da contabilizaÁ„o oficial.
...
// Ientifica o n√∫mero de parcelas
...
If !Empty(c_AdmFin)
	...
	// Verifica de possui a informa√ß√£o no item
	If n_TxAdm = 0
		DbSelectArea("MEN")
		MEN->( DbSetOrder(2))
...
While SE1->(!Eof()) .And. SE1->E1_FILIAL = c_FilE1 .And. SE1->E1_PREFIXO == SF2->F2_PREFIXO .And. SE1->E1_NUM == SF2->F2_DUPL
	...
	If	n_TxAdm > 0 
		...
		//Inicia o controle de transa√ß√£o
		Begin Transaction
			//Chama a rotina autom√°tica
			lMsErroAuto := .F.
			MSExecAuto({|x,y| FINA040(x,y)}, aVetSE1, 3)
			
			//Se houve erro, mostra o erro ao usu√°rio e desarma a transa√ß√£o
			If lMsErroAuto
				MostraErro()
				DisarmTransaction()
			EndIf
