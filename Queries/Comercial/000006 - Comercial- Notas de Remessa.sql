Select 
	 ...
	 RTRIM(B6_CLIFOR) + ' - ' + Rtrim(B6_LOJA) as Cliente_ID,  Rtrim(Cliente.A1_NOME) as Cliente,  Cliente.A1_NREDUZ as Fantasia, F2_CLIENTE as CliEntregaID, A1_NOME as CliEntrega ,
...
Select 
     ...
     Item.R_E_C_N_O_ as Recno,  Rtrim(D2_LOCAL) + ' - ' + RTRIM(Armazem.NNR_DESCRI) as Armazem, 
	 RTRIM(F2_CLIENTE) + ' - ' + Rtrim(F2_LOJA) as Cliente_ID,  Rtrim(Cliente.A1_NOME) as Cliente, Cliente.A1_NREDUZ as Fantasia,F2_CLIENT as CliEntregaID, Cli.A1_NOME as CliEntrega ,
	 RTRIM(Cliente.A1_MUN) as Cidade, Cliente.A1_EST as UF,	 
	 D2_DOC as Nota, F2_SERIE as Serie, D2_TES as TES_ID, F4_TEXTO as TES, D2_TIPO as Tipo, Year(D2_EMISSAO) as Ano,
	 Convert(Varchar(10),Convert(Datetime,D2_EMISSAO,112),103) as Emissao, 
